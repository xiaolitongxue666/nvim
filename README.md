...
| 运行时 | 配置入口 | 安装 |
|--------|----------|------|
| 终端 `nvim` | [init.lua](init.lua) + `lua/plugins/` | `./install.sh` |
...
Windows 可用各子目录下的 `install.cmd`（调用 Git Bash 执行 `install.sh`）。子安装排错见 [TROUBLE_SHOOT.md](TROUBLE_SHOOT.md#可选vscode-neovim--ideavim-安装排错)。
