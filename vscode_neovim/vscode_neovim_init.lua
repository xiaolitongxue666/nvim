...
if vim.g.vscode then
        ...
        vim.api.nvim_create_autocmd("BufEnter", {
            ...
            callback = function(args)
                if vim.b[args.buf] and vim.b[args.buf].vscode_file_explorer then
                    local map_explorer = function(keys, command, desc)
                        vim.keymap.set('n', keys, command, { buffer = args.buf, silent = true, desc = desc })
                    end

                    map_explorer('<space>', '<Cmd>call VSCodeNotify("list.toggleExpand")<CR>', "切换节点展开/折叠")
                    map_explorer('<cr>',    '<Cmd>call VSCodeNotify("list.select")<CR>', "打开文件/目录")
                    map_explorer('<esc>',   '<Cmd>call VSCodeNotify("workbench.action.focusActiveEditorGroup")<CR>', "返回编辑器焦点")
                    map_explorer('q',       '<Cmd>call VSCodeNotify("workbench.action.toggleSidebarVisibility")<CR>', "关闭侧边栏")
                    map_explorer('i',       '<Cmd>call VSCodeNotify("list.focusUp")<CR>', "向上移动")
                    map_explorer('k',       '<Cmd>call VSCodeNotify("list.focusDown")<CR>', "向下移动")

                    map_explorer('a', '<Cmd>call VSCodeNotify("explorer.newFile")<CR>', "新建文件")
                    map_explorer('A', '<Cmd>call VSCodeNotify("explorer.newFolder")<CR>', "新建文件夹")
                    map_explorer('d', '<Cmd>call VSCodeNotify("deleteFile")<CR>', "删除文件")
                    map_explorer('r', '<Cmd>call VSCodeNotify("renameFile")<CR>', "重命名文件")
                    map_explorer('y', '<Cmd>call VSCodeNotify("filesExplorer.copy")<CR>', "复制文件")
                    map_explorer('x', '<Cmd>call VSCodeNotify("filesExplorer.cut")<CR>', "剪切文件")
                    map_explorer('p', '<Cmd>call VSCodeNotify("filesExplorer.paste")<CR>', "粘贴文件")
                    map_explorer('R', '<Cmd>call VSCodeNotify("workbench.files.action.refreshFilesExplorer")<CR>', "刷新资源管理器")
                end
            end
        })
    end
    ...
    vim.keymap.set('n', '<C-n>', '<Cmd>call VSCodeNotify("workbench.action.quickOpenNavigateNext")<CR>', { silent = true })
    vim.keymap.set('n', '<C-p>', '<Cmd>call VSCodeNotify("workbench.action.quickOpenNavigatePrevious")<CR>', { silent = true })
